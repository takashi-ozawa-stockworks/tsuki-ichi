<script lang="ts">
  ...
  // 初期値: URLパラメータがあればそれを使い、なければ今日の日付
  ...
  const handleDateChange = (e: Event) => {
    ...
    if (val) {
      ...
      // URLのクエリパラメータも更新しておくと、リロードしても維持されて便利（任意）
  ...
  // 追加: カレンダーを強制的に開くための参照と関数
  let dateInput: HTMLInputElement;
  
  const openPicker = () => {
    try {
      // ブラウザ標準のカレンダー表示メソッドを実行
      dateInput.showPicker(); 
    } catch (e) {
      // 古いブラウザやSafari(Mac)など未対応の場合のフォールバック
...
<div class="space-y-6 pb-32">
	<div
		class="flex items-center justify-between bg-white p-2 rounded-lg shadow-sm border border-gray-100"
	>
		<button
